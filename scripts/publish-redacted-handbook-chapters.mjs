...
const chapters = [
  ...
  },
  {
    ...
    description: "由去識別完整稿切分出的第 4 章，保留原手冊研究工具、資料處理與輔助資料來源頁。",
    range: [96, 134],
  ...
  },
  {
    ...
    range: [162, 165],
    focus: "備忘、舊設備或聯絡資訊脈絡；含敏感資訊的頁面只保留去識別標記。"
  }
];
...
function renderChapter(chapter, selectedPages) {
  ...
  const output = [
    ...
    `- OCR source pages ${String(start).padStart(3, "0")}-${String(end).padStart(3, "0")}`,
    `- 本章頁數：${pageCount}`,
